...
- [] 新增独立的 Milkdown 编辑器组件，不直接改散在页面里的逻辑。
- [] 让 Milkdown 以 Markdown 字符串作为唯一输入输出，和 `form.content` 双向同步。
- [] 保留当前 `MarkdownRenderer` 作为后台预览和博客发布态渲染标准。
- [] 接入文章图片上传能力，保持上传后插入 Markdown 图片语法。
- [] 迁移保存快捷键、格式化保存快捷键、重做快捷键等编辑器快捷键。
- [] 适配亮色、暗色主题。
- [] 适配桌面端分栏预览、全屏预览和移动端布局。
- [] 验证标题、段落、加粗、斜体、链接、图片、表格、代码块、任务列表等基础语法。
- [] 验证 KaTeX、脚注、Mermaid、提示块、折叠块、标签页等增强语法不会在编辑器中丢失原文。
- [] 对最常用的自定义语法补充 Milkdown 编辑态表现。
- [] 保留源码编辑或原文查看入口，便于处理 Milkdown 暂不支持的复杂语法。
- [] 补充编辑器迁移相关测试或人工验收清单。
