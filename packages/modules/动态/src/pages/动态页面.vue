...
const showMomentDialog = ref(false)
const 动态图片上限 = 20
...
使用保存快捷键({
  enabled: () => showMomentDialog.value && !store.saving,
...
async function handlePublish() {
  ...
  try {
    ...
    momentImages.value = []
    momentImagesExpanded.value = false
    showMomentDialog.value = false
    await loadMoments(1, { silent: true })
  } catch (error) {
...
async function openCreateMomentDialog() {
  if (saveTimeout) {
    window.clearTimeout(saveTimeout)
    saveTimeout = null
  }
  editingMoment.value = null
  showMomentDialog.value = true
  await loadDraft()
}
...
<template>
  <div class="page-container">
    ...
    >
      <template #actions>
        <ElButton type="primary" :icon="Plus" @click="openCreateMomentDialog">
          写动态
        </ElButton>
      </template>
    ...
    <BaseDialog
      v-model="showMomentDialog"
      :title="isEditingPublishedMoment ? '编辑已发布动态' : '写动态'"
      width="760px"
      style="max-width: 96vw"
    >
      <div class="moment-dialog-toolbar">
        <ElTooltip :content="isEditingPublishedMoment ? '退出当前编辑并回到草稿' : '自动获取上次未发布的内容'">
          <ElButton text :icon="RefreshLeft" :loading="loadingDraft" @click="isEditingPublishedMoment ? handleReturnToDraft() : loadDraft()">
