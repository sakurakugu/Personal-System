...
from app.shared.auth.deps import 要求当前用户
...
class 友链权限测试(unittest.TestCase):
    ...
    def test_管理接口仅允许登录用户(self) -> None:
        ...
        for key in protected_routes:
            ...
            self.assertIn(要求当前用户, dependency_calls)
            self.assertIn(get_db, dependency_calls)

    def test_公开接口不要求登录用户(self) -> None:
        route_map = build_route_map()
        public_routes = {
        ...
        for key in public_routes:
            route = route_map[key]
            dependency_calls = {dependency.call for dependency in route.dependant.dependencies}
            self.assertNotIn(要求当前用户, dependency_calls)
