...
@router.post("/login", response_model=设备登录响应, status_code=status.HTTP_201_CREATED)
async def login_device(
    ...
    user = await login_user(
        db,
        登录请求(username=body.username, password=body.password),
    )
    result = await 创建设备会话(
        db,
        user_id=user.id,
...
@router.get("/sessions", response_model=list[设备会话列表项信息])
async def 列出设备会话(
    current_user: 用户 = Depends(获取当前用户),
    current_session: 用户设备会话 | None = Depends(获取当前设备会话可选),
    db: AsyncSession = Depends(get_db),
