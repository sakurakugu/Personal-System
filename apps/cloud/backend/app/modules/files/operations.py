...
async def 构建归档载荷(
    ...
    *,
    ...
    selected_article_images: list[文章图片] = []
    if file_ids:
...
async def 上传文章图片(db: AsyncSession, user: 用户, file: UploadFile) -> FileRead:
    """上传文章图片并在需要时压缩静态位图。"""
