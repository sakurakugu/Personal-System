...
def build_moment_image_path(moment_title: str) -> str:
    """构造动态图片在资源管理器中的展示路径。"""
...
def build_moment_image_file_read(record: MomentImage) -> FileRead:
    """将动态图片模型转换为统一的文件响应。"""
...
def build_moment_image_search_read(record: MomentImage) -> FileSearchItemRead:
    """将动态图片模型转换为搜索结果项。"""
    moment_title = record.moment.title if record.moment is not None and record.moment.title is not None else "未命名动态"
    return FileSearchItemRead(
        id=record.id,
        ...
        mime_type=record.mime_type,
        created_at=record.created_at,
        path=build_moment_image_path(moment_title),
        moment_id=record.moment_id,
        moment_title=moment_title,
    )
