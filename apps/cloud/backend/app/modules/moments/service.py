...
def 应用动态删除状态(moment: 动态, *, now: datetime | None = None) -> None:
    """将动态标记为已删除。"""
    moment.is_deleted = True
    moment.deleted_at = now or datetime.now(timezone.utc)


def 恢复动态删除状态(moment: 动态) -> None:
    """恢复动态删除状态。"""
    moment.is_deleted = False
    moment.deleted_at = None


def 刷新动态最后编辑时间(moment: 动态, *, now: datetime | None = None) -> None:
    """刷新动态最后编辑时间。"""
...
async def 获取公开动态或404(db: AsyncSession, moment_id: str) -> 动态:
    """获取单个已发布动态。"""
...
async def 构建动态公开读取(moment: 动态, *, visitor_id: str | None = None) -> 动态公开信息:
    """构造公开动态响应，并附带点赞状态。"""
    liked = False
    if visitor_id:
...
async def 点赞动态(
    ...
    if changed:
        moment.like_count += 1
        await db.flush()
    return 动态点赞信息(like_count=moment.like_count, changed=changed, liked=True)
...
async def 取消点赞动态(
    ...
    if not visitor_id:
        return 动态点赞信息(like_count=moment.like_count, changed=False, liked=False)

    changed = await 移除集合成员(f"like:moment:{moment.id}", visitor_id)
    if changed:
        moment.like_count = max(0, moment.like_count - 1)
        await db.flush()
    return 动态点赞信息(like_count=moment.like_count, changed=changed, liked=False)
...
async def 记录动态浏览(
    ...
    if changed:
        moment.view_count += 1
        await db.flush()
    return 动态浏览信息(view_count=moment.view_count, changed=changed)


async def 获取草稿(db: AsyncSession, user: 用户) -> 动态 | None:
    """获取当前用户草稿。"""
    result = await db.execute(
        动态查询().where(
            动态.user_id == user.id,
...
async def 发布动态(
    ...
    result = await db.execute(动态查询().where(动态.id == moment.id))
...
async def 列出我的动态(
    ...
    *,
    ...
    if not is_deleted:
        query = query.where(动态.is_published.is_(True)).order_by(动态.published_at.desc())
    else:
...
async def 获取动态或404(db: AsyncSession, moment_id: str) -> 动态:
    """获取单个动态。"""
    result = await db.execute(
        select(动态).where(
            动态.id == moment_id,
...
async def 获取已删动态或404(db: AsyncSession, moment_id: str) -> 动态:
    """获取回收站中的动态。"""
    result = await db.execute(
        select(动态).where(
            动态.id == moment_id,
            动态.is_deleted.is_(True),
        )
    )
    moment = result.scalar_one_or_none()
    if moment is None:
...
async def 列出动态图片存储键(db: AsyncSession, moment_id: UUID) -> list[str]:
    """获取动态关联的全部图片对象键。"""
    result = await db.execute(
        select(动态图片.storage_key).where(动态图片.moment_id == moment_id)
    )
    return list(result.scalars().all())


def 确保动态删除权限(moment: 动态, user: 用户) -> None:
    """校验动态删除权限。"""
    确保动态写入权限(moment, user)


async def 删除动态(db: AsyncSession, moment_id: str, user: 用户, *, permanent: bool) -> None:
    """删除动态。"""
    if permanent:
...
async def 恢复动态(db: AsyncSession, moment_id: str, user: 用户) -> 动态信息:
    """从回收站恢复动态。"""
    moment = await 获取已删动态或404(db, moment_id)
    确保动态删除权限(moment, user)
    恢复动态删除状态(moment)
