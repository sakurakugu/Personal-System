...
def 确保访客ID(request: Request, response: Response) -> str:
    """确保请求具备匿名访客标识。"""
...
def 获取访客ID(request: Request) -> str | None:
    """从请求中读取匿名访客标识。"""
    return request.cookies.get(_VISITOR_COOKIE_NAME)


async def 单次添加集合成员(key: str, member: str) -> bool:
    """向 Redis 集合写入成员，仅在首次写入时返回真。"""
    redis = await get_redis()
    added = await _解析_redis_结果(redis.sadd(key, member))
    return bool(added)


async def 移除集合成员(key: str, member: str) -> bool:
    """从 Redis 集合移除成员，仅在确实移除时返回真。"""
    redis = await get_redis()
    removed = await _解析_redis_结果(redis.srem(key, member))
    return bool(removed)


async def 包含集合成员(key: str, member: str) -> bool:
    """判断 Redis 集合是否包含指定成员。"""
    redis = await get_redis()
    exists = await _解析_redis_结果(redis.sismember(key, member))
    return bool(exists)


async def 单次标记键(key: str, *, expire_seconds: int | None = None) -> bool:
    """按键记录一次性操作，仅在首次写入时返回真。"""
    redis = await get_redis()
    created = await _解析_redis_结果(redis.set(key, "1", nx=True, ex=expire_seconds))
    return bool(created)
